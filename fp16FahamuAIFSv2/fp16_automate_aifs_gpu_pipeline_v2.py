#!/usr/bin/env python3
"""
Automated AIFS GPU Pipeline - v2.0 / FP16 (Half Precision).

Runs the **aifs-ens-2.0** model. Counterpart of
``fp16FahamuAIFSv1/fp16_automate_aifs_gpu_pipeline.py`` — same per-member
download → inference → upload → cleanup loop, but:
  * checkpoint ``ecmwf/aifs-ens-2.0`` (loaded once),
  * imports the v2 runner ``fp16_multi_run_AIFS_ENS_v2``,
  * default GCS input  ``<date>/input_v2/``  (the 112-field v2 pkls),
  * default GCS output ``<date>/fp16_v2_forecasts/``.
...
For each member:
    ...
    3. Upload .grib files to GCS (fp16_v2_forecasts/)
    4. Cleanup: delete BOTH .pkl AND .grib files locally
    → Repeat for next member

Usage:
    python fp16FahamuAIFSv2/fp16_automate_aifs_gpu_pipeline_v2.py --date 20251127_0000 --members 1-50

GCS Structure:
    gs://aifs-aiquest-us-20251127/
        YYYYMMDD_0000/
            input_v2/           <- 112-field v2 pickle files
            fp16_v2_forecasts/  <- v2 FP16 GRIB output files
"""
...
# GCS output path (v2.0 — distinct from v1's fp16_forecasts/)
GCS_OUTPUT_SUBPATH = "fp16_v2_forecasts"

# GCS input default subfolder (the 112-field v2 pkls live under <date>/input_v2/)
GCS_INPUT_SUBPATH = "input_v2"
...
def download_single_member(member: int, date_str: str, input_dir: str,
    ...
    if gcs_input_prefix:
        ...
        gcs_blob_name = f"{date_str}/{GCS_INPUT_SUBPATH}/input_state_member_{member:03d}.pkl"
...
def run_single_member(member: int, input_dir: str, output_dir: str,
    ...
    import fp16_multi_run_AIFS_ENS_v2
    from fp16_multi_run_AIFS_ENS_v2 import run_ensemble_member

    # Override the hardcoded PICKLE_INPUT_DIR and LEAD_TIME with our values
    fp16_multi_run_AIFS_ENS_v2.PICKLE_INPUT_DIR = input_dir
    fp16_multi_run_AIFS_ENS_v2.LEAD_TIME = lead_time
...
def upload_single_member(member: int, output_files: List[str], date_str: str,
    ...
    """Upload GRIB files for a single ensemble member to fp16_v2_forecasts/."""
    output_subpath = gcs_output_subpath or GCS_OUTPUT_SUBPATH
    if not output_files:
        print(f"    [UPLOAD] No files to upload")
        return True

    # Prepare file list - upload to fp16_v2_forecasts/ (or custom subpath)
    file_list = []
...
def run_pipeline(date_str: str, members: List[int],
    ...
    if gcs_input_prefix:
        ...
        print(f"GCS Input:  gs://{bucket}/{date_str}/{GCS_INPUT_SUBPATH}/")
    ...
    print("\nLoading AIFS-ENS v2.0 model (FP16 precision)...")
    ...
    print("Model loaded successfully (v2.0, FP16 mode)\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Automated AIFS GPU Pipeline - v2.0 / FP16 (Half Precision)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
MODEL:
  ecmwf/aifs-ens-2.0, FP16. Consumes the 112-field v2 pkls produced by
  ecmwf_opendata_pkl_input_aifsens_v2.py.

PROCESSING MODE:
  Each ensemble member is fully processed before moving to the next:
    1. Download single .pkl file (from <date>/input_v2/)
    2. Run GPU inference (FP16) → generate .grib files
    3. Upload .grib files to GCS (fp16_v2_forecasts/)
    4. Cleanup: delete BOTH .pkl AND .grib files locally

GCS OUTPUT:
  Files are uploaded to: gs://{bucket}/{date}/fp16_v2_forecasts/

Examples:
  python fp16FahamuAIFSv2/fp16_automate_aifs_gpu_pipeline_v2.py --date 20251127_0000 --members 1-50
  python fp16FahamuAIFSv2/fp16_automate_aifs_gpu_pipeline_v2.py --date 20251127_0000 --members 1,5,10
        """
    )
    ...
    parser.add_argument('--gcs-input-prefix', default=None,
                       help='Custom GCS input prefix (e.g., 20260305_0000/input_v2). '
                            'Overrides default {date}/input_v2/ path.')
